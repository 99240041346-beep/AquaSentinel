...
def auth(f):
 ...
 def w(*a,**k):
  ...
  return f(*a,**k)
 return w
def device_ok(): return bool(os.getenv("DEVICE_API_KEY")) and secrets.compare_digest(request.headers.get("X-Device-Key",""),os.getenv("DEVICE_API_KEY"))
def cfg():
 d=DEFAULTS.copy()
 for x in Setting.query.all(): d[x.key]=x.value
 return d
def iso(dt): return dt.astimezone(timezone.utc).isoformat() if dt.tzinfo else dt.replace(tzinfo=timezone.utc).isoformat()
def rd(x): return {"id":x.id,"temperature":x.temperature,"ph":x.ph,"turbidity":x.turbidity,"created_at":iso(x.created_at)}
def evaluate(r,c):
 out=[]
...
def latest():
 ...
 return jsonify({"reading":rd(x) if x else None,"settings":cfg()})
@app.get("/api/alerts")
@auth
def api_alerts():
